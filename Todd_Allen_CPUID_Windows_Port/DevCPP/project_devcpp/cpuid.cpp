...
** 2020 by Todd Allen.
**
** Linux version of this cpuid utility ported to Windows
** (compatible with Dev-C++ 5.11 and Visual Studio 2019)
...
/*
**
** Comments to original linux version source by Todd Allen.
**
** cpuid dumps CPUID information for each CPU.
** Copyright 2003,2004,2005,2006,2010,2011,2012,2013,2014,2015,2016,2017,2018,
** 2020 by Todd Allen.
...
// this definition required for compatibility with legacy strings support functions with unrestricted pointers
// must be located BEFORE headers
...
// version strings
#define VERSTR1 "Cpuid utility dumps CPUID information for each CPU. By Todd Allen. Original version: 20201006."
#define VERSTR2 "Microsoft Windows port by Ilya Manusov. Port version v0.01.01 (October, 31, 2020)."

#if defined(__GNUC__)
// Version message strings for Dev-C++ 5.11
#if defined _WIN64
#define VERSTR3 "Build type: x64 (x86-64). Dev-C++ 5.11."
#elif defined _WIN32
#define VERSTR3 "Build type: ia32 (x86). Dev-C++ 5.11."
#else
#define VERSTR3 "Unknown build type, can be not compatible with x86, x86-64."
#endif
// Version message strings for Visual Studio 2019
#else
#if defined _WIN64
#define VERSTR3 "Build type: x64 (x86-64). Visual Studio 2019."
#elif defined _WIN32
#define VERSTR3 "Build type: ia32 (x86). Visual Studio 2019."
#else
#define VERSTR3 "Unknown build type, can be not compatible with x86, x86-64."
#endif
#endif

// version text block as printf argument
#define VERSION_TEXT "%s\n%s\n%s\n", VERSTR1, VERSTR2, VERSTR3

// this definition required for supress unused arguments warning, debug purpose
#if defined(__GNUC__)
...
// descriptor for supported option
struct option {
    const char* name;   // full name string of option, NULL means end of options list
    int has_arg;        // arguments usage mode, see below
    int* flag;          // reserved for return matched char, yet not used
    int val;            // single char representation for option
};

// arguments usage modes encodings
#define no_argument            0
#define required_argument      1
#define optional_argument      2

// boolean type encoded as integer
typedef int   intbool;
#define TRUE  1
#define FALSE 0

// strings and pointers representation
typedef char* string;                 // variable pointer to variable string
typedef const char* cstring;          // variable pointer to constant string
typedef const char* const ccstring;   // constant pointer to constant string

// representation of strcmp function return when compared strings matched
#define SAME  0

// macro for find maximum of l=left and r=right arguments
#ifndef MAX
#define MAX(l,r)       ((l) > (r) ? (l) : (r))
#endif

// macro for calculate number of elements in the array
#define LENGTH(array)  (sizeof(array) / sizeof(array[0]))

// bits per integer, typical 32 for 32- and 64-bit Linux and Windows compilers
#define BPI  32

// raising 2 to the power of x=argument
/*
#define POWER2(power) \
   (1 << (power))
*/
#define POWER2(power) \
   ((unsigned __int64)1 << (power))

// calculate bitmask: all "1" if width >= BPI, or by width if width < BPI
#define RIGHTMASK(width) \
   (((width) >= BPI) ? ~0 : POWER2(width)-1)

// extract bitfield by masking and shift
// value = integer 32-bit value contain extracted bitfield
// start = extracted bitfield low bit position, 0-based, inclusive, works as count of right shifts
// after = extracted bitfield high bit position, 0-based, exclusive, works as bitmask
/*
#define BIT_EXTRACT_LE(value, start, after) \
   (((value) & RIGHTMASK(after)) >> start)
*/
#define BIT_EXTRACT_LE(value, start, after) \
   (int)(((value) & RIGHTMASK(after)) >> start)

// double word number for each register at one CPUID function call result: EAX, EBX, ECX, EDX
// this is indexes for array of 4 integers
#define WORD_EAX  0
#define WORD_EBX  1
#define WORD_ECX  2
#define WORD_EDX  3

// number of 32-bit double words at one CPUID function call result: EAX, EBX, ECX, EDX
// used array of 4 integers
#define WORD_NUM  4

// program context, include variables required for linux context emulation on windows port, 
// some of this variables yet not used
cstring program = NULL;         // pointer to this application exe file name string, for messages
cstring emulate_optarg = NULL;  // pointer to option additional arguments, setup when parse command line
int emulate_opterr = 0;         // flag for enable print error messages by parser, enabled when nonzero
int emulate_optind = 1;         // index of current parsed command line argument, starts with 0 = exe file name
int emulate_errno = 0;          // last detected error number 
int emulate_optopt = 0;         // last detected option character

// find substring (regular expression build by string needle) in the string (haystack)
static intbool strregexp(const char* haystack, const char* needle)
{
    std::regex rex(needle);
    return std::regex_search(haystack, rex);
}

// structure for parameter show, parameters represented as bitfield of some data value
typedef struct {
    ccstring      name;       // parameter name
    unsigned int  low_bit;    // low bit of this extracted parameter bitfield, 0-based, inclusive
    unsigned int  high_bit;   // high bit of this extracted parameter bitfield, 0-based, inclusive
    ccstring* images;         // pointer to array of decode strings or special code, see below
} named_item;

// special codes for parameter decoding control
// not listed values interpreted as pointer to array of decode strings
#define NIL_IMAGES     (ccstring*)NULL   // decode string absent, show as "name = value" only
#define MINUS1_IMAGES  (ccstring*)1      // for parameters, represented as value-1, show as "name = value+1"
#define X2_IMAGES      (ccstring*)2      // for parameters, represented as value*2, show as "name = value/2"

// return maximum parameter name length for text formatting
// accept array of named_item structures (names) and this array number of elements (length)
static unsigned int
get_max_len(named_item names[], unsigned int length)
{
    unsigned int  result = 0;
    unsigned int  i;

    for (i = 0; i < length; i++) {
        result = MAX(result, (unsigned int)strlen(names[i].name));  // cycle for find maximum
    }

    return result;
}

// show (printf) bitfields sequence with field-specific formats, interpreting named_item structures
// value   = data value for extract target bitfield
// names[] = array of parameters control structures, select extracted bitfields, see above
// length  = number of parameters for extract
// max_len = parameter name maximum length for string formatting
static void
print_names(unsigned int value, named_item names[], unsigned int length, unsigned int max_len)
{
    unsigned int  i;
    ...
    for (i = 0; i < length; i++) {
        unsigned int  field = BIT_EXTRACT_LE(value,
            names[i].low_bit,
            names[i].high_bit + 1);           // +1 for make bitfield inclusive, note macro parameter is exclusive
        if (names[i].images == X2_IMAGES) {   // for parameters, represented as X*2
            printf("      %-*s = %.1f\n",
                max_len,
                names[i].name,
                (double)field / 2.0);
        }
        else if (names[i].images == MINUS1_IMAGES) {   // for parameters, represented as X-1
            ...
            || names[i].images[field] == NULL) {   // for parameters, visualized simple as "name = value"
            printf("      %-*s = 0x%0x (%u)\n",
                max_len,
                names[i].name,
                field,
        ...
        else {   // for parameters, represented as index of decode strings array, index=field, array=images
            printf("      %-*s = %s\n",
                max_len,
                names[i].name,
...
// bools naming, 0=false, 1=true
static ccstring  bools[] = { "false",
                             "true" };

// processors vendors enumeration, 0 means processor vendor not recognized
typedef enum {
    VENDOR_UNKNOWN,
    VENDOR_INTEL,
    VENDOR_AMD,
...
// virtual machines monitors (hypervisors or virtual processors) enumeration,  0 means VMM vendor not recognized
typedef enum {
    HYPERVISOR_UNKNOWN,
    HYPERVISOR_VMWARE,
    HYPERVISOR_XEN,
...
// bits masks for TFMS (Type, Family, Model, Stepping) fields selection in the Processor Signature
// bitfields positions:
// stepping = [03-00]
// model    = [07-04]
// family   = [11-08]
...
// reserved        = [31-28]
// for example, mask = 0x0ff00f00 select extended family + family fields
...
// shifts count for Type, Family, Model, Stepping, Extended Family, Extended Model
// number of left shifts for positioning value to appropriate bitfield
#define _T(v)      ((v) << 12)
#define _F(v)      ((v) << 8)
#define _M(v)      ((v) << 4)
#define _S(v)      (v)
...
// get masked value = low byte, get simple value
#define __B(v)     ((v) & 0x000000ff)
#define _B(v)      (v)

// Family, Model, Stepping (include extended) combined values,
// positioning arguments to bitfields by shifts, then add shifted arguments
#define _FM(xf,f,xm,m)     (_XF(xf) + _F(f) + _XM(xm) + _M(m))
#define _FMS(xf,f,xm,m,s)  (_XF(xf) + _F(f) + _XM(xm) + _M(m) + _S(s))

// building blocks for series of processor detection by Type, Family, Model, Stepping
// each block contain detection criteria and actions, executed if detection criteria true
// sequence of comparings compiled as sequence of if-else constructions,
// recommended refactoring to array of criteria descriptors
#define START \
   if (0)
#define F(xf,f,...) \
...
// data structure for processor detecting, represents vendor-specific and device-specific facts
// this structure accumulate data when CPUID functions execution and used as argument for
// processor model string build plus additional summary information generation
// see details at processors documentation about CPUID functions output parameters
typedef struct {
    vendor_t       vendor;                       // processor vendor as numeric id by vendor_t enumeration
    intbool        saw_4;                        // flag CPUID function 04h results valid, can be used for multiprocessor enumeration
    intbool        saw_b;                        // flag CPUID function 0Bh results valid, can be used for multiprocessor enumeration
    intbool        saw_1f;                       // flag CPUID function 1Fh results valid, can be used for multiprocessor enumeration
    unsigned int   val_0_eax;                    // CPUID function 00h output EAX = Maximum supported standard CPUID function
    unsigned int   val_1_eax;                    // CPUID function 01h output EAX = Processor Signature: Type, Family, Model, Stepping
    unsigned int   val_1_ebx;                    // CPUID function 01h output EBX = Brand Index / CLFLUSH size / Maximum ID / Initial APIC ID
    unsigned int   val_1_ecx;                    // CPUID function 01h output ECX = Features bitmap
    unsigned int   val_1_edx;                    // CPUID function 01h output EDX = Features bitmap
    unsigned int   val_4_eax;                    // CPUID function 04h output EAX = Cache Type / Level / Flags / Max. for cache / Max. for packegs
    unsigned int   val_b_eax[2];                 // CPUID function 0Bh subfunctions 0-1 output EAX = x2APIC ID bits shifts
    unsigned int   val_b_ebx[2];                 // CPUID function 0Bh subfunctions 0-1 output EBX = Number of logical processors at this level
    unsigned int   val_1f_eax[6];                // CPUID function 1Fh subfunctions 0-5 output EAX = x2APIC ID bits shifts
    unsigned int   val_1f_ebx[6];                // CPUID function 1Fh subfunctions 0-5 output EBX = Number of logical processors at this level
    unsigned int   val_1f_ecx[6];                // CPUID function 1Fh subfunctions 0-5 output ECX = Level number
    unsigned int   val_80000001_eax;             // CPUID function 80000001h output EAX = Processor Extended Signature: Type, Family, Model, Stepping
    unsigned int   val_80000001_ebx;             // CPUID function 80000001h output EBX = Brand Identifier, package type
    unsigned int   val_80000001_ecx;             // CPUID function 80000001h output ECX = Extended features bitmap
    unsigned int   val_80000001_edx;             // CPUID function 80000001h output EDX = Extended features bitmap
    unsigned int   val_80000008_ecx;             // CPUID function 80000008h output ECX = Perf. TSC size / APIC ID size / Number of threads
    unsigned int   val_8000001e_ebx;             // CPUID function 8000001Eh output EBX = Threads per core / Core ID
    unsigned int   transmeta_proc_rev;           // CPUID function 80860002h output EAX = Transmeta processor revision
    char           brand[48 + 1];                // Processor brand string, results of CPUID 80000002h-80000004h or synthetical generated
    char           transmeta_info[64 + 1];       // results of CPUID 80860003h-80860006h = Transmeta information string
    char           override_brand[48 * 2 + 1];   // storage for synthetical generated brand string
    char           soc_brand[48 + 1];            // SoC brand string, results of CPUID function 00000017h subfunctions 1-3
    hypervisor_t   hypervisor;                   // hypervisor (VMM) vendor as numeric id by hypervisor_t enumeration

    struct mp {
        const char* method;           // text string about MP detection method
        unsigned int   cores;         // core count
        unsigned int   hyperthreads;  // threads per core count
    } mp;

    struct br {
        intbool    mobile;

        struct /* Intel */ {        // boolean flags = f(model name text string parse)
            intbool    celeron;
            intbool    core;
            intbool    pentium;
            intbool    atom;
        ...
        struct /* AMD */ {        // boolean flags = f(model name text string parse)
            intbool    athlon_lv;
            intbool    athlon_xp;
            intbool    duron;
            intbool    athlon;
        ...
        struct /* Cyrix */ {        // boolean flags = f(model name text string parse)
            intbool    mediagx;
        };
        struct /* VIA */ {          // boolean flags = f(model name text string parse)
            intbool    c7;
            intbool    c7m;
            intbool    c7d;
            intbool    eden;
    ...
    struct bri {                    // boolean flags = f(Intel Brand Index), use CPUID function 01h, EBX bits [7-0]
        intbool  desktop_pentium;
        intbool  desktop_celeron;
        intbool  mobile_pentium;
        intbool  mobile_pentium_m;
        intbool  mobile_celeron;
        intbool  xeon_mp;
        intbool  xeon;
    } bri;
    ...
    /* ----------------------  ---------------  */
    intbool       L2_4w_1Mor2M;  /* Xeon                    Xeon             */
    intbool       L2_4w_512K;    /* normal, Mobile or Xeon  normal or Xeon   */
    intbool       L2_4w_256K;    /* Mobile                   -               */
    intbool       L2_8w_1Mor2M;  /*  -                      Xeon             */
    intbool       L2_8w_512K;    /*  -                      normal           */
    intbool       L2_8w_256K;    /*  -                      normal or Xeon   */
                  /* none */     /* Celeron                  -               */

    intbool       L2_2M;         /* Nocona lacks, Irwindale has */
                                 /* Conroe has more, Allendale has this */
    intbool       L2_6M;         /* Yorkfield C1/E0 has this, M1/R0 has less */
    intbool       L3;            /* Cranford lacks, Potomac has */

    intbool       L2_256K;       /* Barton has more, Thorton has this */
    intbool       L2_512K;       /* Toledo has more, Manchester E6 has this */
} code_stash_t;

// duplicated, also defined inside code_stash_t, this required for Visual Studio compiler ?
struct mp {
    const char* method;
    unsigned int   cores;
...
// empty data structure for processor detection
// this structure accumulate data when CPUID functions execution and used as argument for
// processor model string build plus additional summary information generation
// before this procedures it must be blank = NIL_STASH
...
// detect AMD processor model use vendor-specific and device-specific information
// this is only part of detection sequence, no K5/K6 support at this part
// stash =    = collection of vendor-specific and device-specific information after CPUID functions execution
// brand_pre  = pointer to pointer to generated string: prefix part of processor name
// brand_post = pointer to pointer to generated string: postfix part of processor name
// proc       = pointer for build and return processor name string
static void
decode_amd_model(const code_stash_t* stash, const char** brand_pre, const char** brand_post, char* proc)
{
    *brand_pre = NULL;
    *brand_post = NULL;
    *proc = '\0';
...
// override AMD brand string
// manipulate with strings inside stash structure
// target updated string is stash->override_brand
static void
...
// print brand string with warning about override
// use string inside stash structure
static void
...
// detect processor cache size by descriptors analysing, for INTEL style CPUID function 00000002h
// stash = pointer to structure for accumulate processor information
// value = one descriptor byte from bytes sequence, available by cpuid function 00000002h
// for this function, accumulated information represented as set of binary flags (set TRUE or FALSE)
// function must be called in cycle for all cache descriptor bytes at results of CPUID function 00000002h
static void
stash_intel_cache(code_stash_t* stash, unsigned char value)
{
    switch (value) {
...
// decode Brand ID value, for INTEL processors
// stash = pointer to structure for accumulate processor information,
// both as source and destination data for this function,
// for this function, accumulated information represented as set of binary flags (set TRUE or FALSE)
static void
...
// decode brand string (CPU model name), recognize vendor-specific and model-specific facts,
// search patterns in the model name string, use regular expressions
// brand = pointer to argument string, processor name for parse
// stash = pointer to structure for accumulate processor information,
// for this function, accumulated information represented as set of binary flags (set TRUE or FALSE)
static void
decode_brand_string(const char* brand, code_stash_t* stash)
{
...
// decode brand string (CPU model name), use override logic
// stash = pointer to structure for accumulate processor information
// first try analyse override string variant, if absent, analyse default string variant
static void
...
*/
// stash = pointer to structure for accumulate processor information
// return TRUE if processor model criteria detected, otherwise FALSE
static intbool is_amd_egypt_athens_8xx(const code_stash_t* stash)
{
    /*
...
// decode processor vendor
// note absent "CentaurHauls"
// vendor = numeric vendor code by enumeration
// stash = pointer to structure for accumulate processor information, here used as additional argument
// return vendor name string
static cstring
decode_vendor(vendor_t vendor, const code_stash_t* stash)
{
    switch (vendor) {
...
// structure for descript architecture
typedef struct {
    ...
    intbool  core_is_uarch; // for some uarches, the core names are based on the
                            // uarch names, so the uarch name becomes redundant
} arch_t;

// blank architecture fields
// arch = pointer to structure
static void
...
// decode microarchitecture for INTEL
// val  = input value, processor TFMS signature, CPUID function 00000001h, register EAX output
// arch = pointer to structure for accumulate processor information
static void
decode_uarch_intel(unsigned int val, arch_t* arch)
{
    init_arch(arch);
    ...
    cstring* p = &arch->phys;
    intbool* ciu = &arch->core_is_uarch;
...
// decode microarchitecture for AMD
// bug: 00000500h is K5
// F(0, 5, *u = "K6");
// early detection of 00000500h required before this string ?
// val  = input value, processor TFMS signature
// arch = pointer to structure for accumulate processor information
static void
decode_uarch_amd(unsigned int val, arch_t* arch)
{
    init_arch(arch);
...
// decode microarchitecture for CYRIX
// val  = input value, processor TFMS signature
// arch = pointer to structure for accumulate processor information
static void
decode_uarch_cyrix(unsigned int val, arch_t* arch)
{
    init_arch(arch);
...
// decode microarchitecture for VIA
// val  = input value, processor TFMS signature
// arch = pointer to structure for accumulate processor information
// stash = pointer to structure for accumulate processor information, here used as additional argument
static void
decode_uarch_via(unsigned int val, arch_t* arch, const code_stash_t* stash)
{
    init_arch(arch);
...
// decode microarchitecture for UMC
// val  = input value, processor TFMS signature
// arch = pointer to structure for accumulate processor information
static void
decode_uarch_umc(unsigned int val, arch_t* arch)
{
    init_arch(arch);
...
// decode microarchitecture for NEXGEN
// val  = input value, processor TFMS signature
// arch = pointer to structure for accumulate processor information
static void
decode_uarch_nexgen(unsigned int val, arch_t* arch)
{
    init_arch(arch);
...
// decode microarchitecture for RISE
// val  = input value, processor TFMS signature
// arch = pointer to structure for accumulate processor information
static void
decode_uarch_rise(unsigned int val, arch_t* arch)
{
    init_arch(arch);
...
// decode microarchitecture for TRANSMETA
// val  = input value, processor TFMS signature
// arch = pointer to structure for accumulate processor information
static void
decode_uarch_transmeta(unsigned int val, arch_t* arch)
{
    init_arch(arch);
...
// decode microarchitecture for SIS
// val  = input value, processor TFMS signature
// arch = pointer to structure for accumulate processor information
static void
decode_uarch_sis(unsigned int val, arch_t* arch)
{
    init_arch(arch);
...
// decode microarchitecture for HYGON
// val  = input value, processor TFMS signature
// arch = pointer to structure for accumulate processor information
static void
decode_uarch_hygon(unsigned int val, arch_t* arch)
{
    init_arch(arch);
...
// decode microarchitecture for ZHAOXIN
// val  = input value, processor TFMS signature
// arch = pointer to structure for accumulate processor information
static void
decode_uarch_zhaoxin(unsigned int val, arch_t* arch)
{
    init_arch(arch);
...
// detect vendor and decode microarchitecture for detected vendor
// val    = input value, processor TFMS signature, CPUID function 00000001h, register EAX output
// vendor = vendor numeric id by enumeration
// stash  = pointer to structure for accumulate processor information, here used as additional argument
// arch   = pointer to structure for accumulate processor information
static void
decode_uarch(unsigned int val, vendor_t vendor, const code_stash_t* stash, arch_t* arch)
{
    init_arch(arch);   // pre-blank results

    switch (vendor) {  // select appropriate vendor-specific decoder
    case VENDOR_INTEL:
    ...
    case VENDOR_ZHAOXIN:
        ...
        break;
    case VENDOR_NSC:      // vendor-specific decoders yet absent for this vendors
    case VENDOR_VORTEX:
    case VENDOR_RDC:
    case VENDOR_UNKNOWN:
        break;
...
// print vendor, microarchitecture and additional parameters
// stash = pointer to structure for accumulate processor information
static void
...
// build and return string with CPU microarchitecture and additional parameters
// synth  = source string, some of details can be "unknown"
// val    = processor TFMS signature
// vendor = vendor numeric id by enumeration
// return string with CPU microarchitecture and additional parameters, add details to source string
static cstring
append_uarch(ccstring synth, unsigned int val, vendor_t vendor)
{
    if (synth != NULL) {
        arch_t  arch;
...
// helper procedure, part of CPU signature detection for return CPU name string
// can refactor: use criteria descriptors instead if-else sequence by FM and similar macro
// val   = CPU TFMS signature
// stash = pointer to structure for accumulate processor information
static cstring
decode_synth_intel_part_1(unsigned int val,  /* val_1_eax */ const code_stash_t* stash)
{
    cstring  result = NULL;
...
// stash = pointer to structure for accumulate processor information
static cstring
decode_synth_intel_part_2(unsigned int val,  /* val_1_eax */ const code_stash_t* stash)
{
    cstring  result = NULL;
...
// stash = pointer to structure for accumulate processor information
static cstring
decode_synth_intel_part_3(unsigned int val,  /* val_1_eax */ const code_stash_t* stash)
{
    cstring  result = NULL;
...
// stash = pointer to structure for accumulate processor information
static cstring
decode_synth_intel_part_4(unsigned int val,  /* val_1_eax */ const code_stash_t* stash)
{
    cstring  result = NULL;
...
// stash = pointer to structure for accumulate processor information
static cstring
decode_synth_intel_part_5(unsigned int val,  /* val_1_eax */ const code_stash_t* stash)
{
    cstring  result = NULL;
...
// stash = pointer to structure for accumulate processor information
static cstring
decode_synth_intel_part_6(unsigned int val,  /* val_1_eax */ const code_stash_t* stash)
{
    cstring  result = NULL;
...
// stash = pointer to structure for accumulate processor information
static cstring
decode_synth_intel_part_7(unsigned int val,  /* val_1_eax */ const code_stash_t* stash)
{
    cstring  result = NULL;
...
// return processor name string = f(val, stash), for INTEL
// can refactor: use criteria descriptors instead if-else sequence by FM and similar macro
// val   = CPU TFMS signature
// stash = pointer to structure for accumulate processor information
static cstring
decode_synth_intel(unsigned int val,  /* val_1_eax */ const code_stash_t* stash)
{
    cstring  result = NULL;

    result = decode_synth_intel_part_1(val, stash);
    if (result != NULL) return result;
    result = decode_synth_intel_part_2(val, stash);
    if (result != NULL) return result;
    result = decode_synth_intel_part_3(val, stash);
    if (result != NULL) return result;
    result = decode_synth_intel_part_4(val, stash);
    if (result != NULL) return result;
    result = decode_synth_intel_part_5(val, stash);
    if (result != NULL) return result;
    result = decode_synth_intel_part_6(val, stash);
    if (result != NULL) return result;
    result = decode_synth_intel_part_7(val, stash);
    if (result != NULL) return result;
    return "unknown";
}
...
// stash = pointer to structure for accumulate processor information
static cstring
decode_synth_amd_part_1(unsigned int val, const code_stash_t* stash)
{
    cstring  result = NULL;
...
// stash = pointer to structure for accumulate processor information
static cstring
decode_synth_amd_part_2(unsigned int val, const code_stash_t* stash)
{
    cstring  result = NULL;
...
// stash = pointer to structure for accumulate processor information
static cstring
decode_synth_amd_part_3(unsigned int val, const code_stash_t* stash)
{
    cstring  result = NULL;
...
// stash = pointer to structure for accumulate processor information
static cstring
decode_synth_amd_part_4(unsigned int val, const code_stash_t* stash)
{
    cstring  result = NULL;
...
// stash = pointer to structure for accumulate processor information
static cstring
decode_synth_amd_part_5(unsigned int val, const code_stash_t* stash)
{
    cstring  result = NULL;
...
// stash = pointer to structure for accumulate processor information
static cstring
decode_synth_amd_part_6(unsigned int val, const code_stash_t* stash)
{
    cstring  result = NULL;
...
// helper for return processor name string = f(val, stash), for AMD
// can refactor: use criteria descriptors instead if-else sequence by FM and similar macro
// val   = CPU TFMS signature
// stash = pointer to structure for accumulate processor information
static cstring
decode_synth_amd_helper(unsigned int val, const code_stash_t* stash)
{
    cstring  result = NULL;

    result = decode_synth_amd_part_1(val, stash);
    if (result != NULL) return result;
    result = decode_synth_amd_part_2(val, stash);
    if (result != NULL) return result;
    result = decode_synth_amd_part_3(val, stash);
    if (result != NULL) return result;
    result = decode_synth_amd_part_4(val, stash);
    if (result != NULL) return result;
    result = decode_synth_amd_part_5(val, stash);
    if (result != NULL) return result;
    result = decode_synth_amd_part_6(val, stash);
    if (result != NULL) return result;
    return "unknown";
}

// return processor name string = f(val, stash), for AMD
// val   = CPU TFMS signature
// stash = pointer to structure for accumulate processor information
static cstring
decode_synth_amd(unsigned int val, const code_stash_t* stash)
{
    cstring  result = decode_synth_amd_helper(val, stash);
...
// return processor name string = f(val, stash), for CYRIX
// val   = CPU TFMS signature
// stash = pointer to structure for accumulate processor information
static cstring
decode_synth_cyrix(unsigned int val, const code_stash_t* stash)
{
    cstring  result = NULL;
...
// return processor name string = f(val, stash), for VIA
// val   = CPU TFMS signature
// stash = pointer to structure for accumulate processor information
static cstring
decode_synth_via(unsigned int val, const code_stash_t* stash)
{
    cstring  result = NULL;
...
// return processor name string = f(val), for UMC
// val   = CPU TFMS signature
static cstring
...
// return processor name string = f(val), for NEXGEN
// val   = CPU TFMS signature
static cstring
...
// return processor name string = f(val), for RISE
// val   = CPU TFMS signature
static cstring
...
// return processor name string = f(val, stash), for TRANSMETA
// val   = CPU TFMS signature
// stash = pointer to structure for accumulate processor information
static cstring
decode_synth_transmeta(unsigned int val, const code_stash_t* stash)
{
    cstring  result = NULL;
...
// return processor name string = f(val), for SIS
// val   = CPU TFMS signature
static cstring
...
// return processor name string = f(val), for NSC
// val   = CPU TFMS signature
static cstring
...
// return processor name string = f(val), for VORTEX
// val   = CPU TFMS signature
static cstring
...
// return processor name string = f(val), for RDC
// val   = CPU TFMS signature
static cstring
...
// return processor name string = f(val), for HYGON
// val   = CPU TFMS signature
static cstring
...
// return processor name string = f(val), for ZHAOXIN
// val   = CPU TFMS signature
static cstring
...
// return processor models name string = f(val), for AMD
// val = CPU TFMS signature, extended CPUID function 80000001h, register EAX output
static cstring
...
// print processor name string = f(val), for AMD
// val = CPU TFMS signature, extended CPUID function 80000001h, register EAX output
static void
...
// print processor name string = f(val), for HYGON
// val = CPU TFMS signature, extended CPUID function 80000001h, register EAX output
static void
...
// return processor name string = f(val), for VIA
// val = CPU TFMS signature, extended CPUID function 80000001h, register EAX output
static cstring
...
// print processor name string = f(val), for VIA
// val = CPU TFMS signature, extended CPUID function 80000001h, register EAX output
static void
...
// return processor name string = f(val), include CPU vendor selection logic
// val_eax = CPU TFMS signature
// vendor  = processor vendor numeric id by enumeration
// stash   = pointer to structure for accumulate processor information
static cstring
decode_synth(unsigned int val_eax, vendor_t vendor, const code_stash_t* stash)
{
    cstring  synth = NULL;
...
// print CPU name string with text prefix, not use stash information
// val_eax = CPU TFMS signature
// vendor  = processor vendor numeric id by enumeration
static void
print_synth_simple(unsigned int val_eax, vendor_t vendor)
{
...
// print CPU name string with text prefix, use stash information
// stash = pointer to structure for accumulate processor information
static void
...
// macro for CPU family and model extract from value
...
// macro for multiprocessing topology information extract
...
// analysing and add multiprocessing topology information at stash structure fields
// stash = pointer to structure for accumulate processor information
    ...
    case VENDOR_INTEL:
        ...
        */
        if (stash->saw_1f) {
            ...
            unsigned int  tryX;
            for (tryX = 0; tryX < LENGTH(stash->val_1f_ecx); tryX++) {
                if (GET_V2_TOPO_LEVEL(stash->val_1f_ecx[tryX]) == V2_TOPO_SMT) {
                    stash->mp.hyperthreads
                        = GET_V2_TOPO_PROCESSORS(stash->val_1f_ebx[tryX]);
                }
                else if (GET_V2_TOPO_LEVEL(stash->val_1f_ecx[tryX]) == V2_TOPO_CORE) {
                    stash->mp.cores = GET_V2_TOPO_PROCESSORS(stash->val_1f_ebx[tryX]);
                }
            }
...
// print multiprocessing (MP) topology information
// mp = pointer to structure 
...
// this method changed
// calculate number of bits to represent given value
// input = v, output = minimum number of bits required to represent v
...
// macro for extract data from results of CPUID functions 0Bh, 1Fh.
...
// print multiprocessing topology (APIC, SMP, SMT) information
// stash = pointer to structure for accumulate processor information
    ...
    case VENDOR_INTEL:
        ...
        */
        if (stash->saw_1f) {
            unsigned int  tryX;
            for (tryX = 0; tryX < LENGTH(stash->val_1f_ecx); tryX++) {
                unsigned int  level = GET_V2_TOPO_LEVEL(stash->val_1f_ecx[tryX]);
                if (level == V2_TOPO_SMT) {
                    smt_width = GET_V2_TOPO_WIDTH(stash->val_1f_eax[tryX]);
                }
                else if (level == V2_TOPO_CORE) {
                    core_width = GET_V2_TOPO_WIDTH(stash->val_1f_eax[tryX]);
                }
            }
...
// print some features for AMD processors
// stash = pointer to structure for accumulate processor information
static void print_instr_synth_amd(code_stash_t* stash)
{
    intbool  cmpxchg8b = (BIT_EXTRACT_LE(stash->val_80000001_edx, 8, 9)
        || BIT_EXTRACT_LE(stash->val_1_edx, 8, 9));
    intbool  cond = (BIT_EXTRACT_LE(stash->val_80000001_edx, 15, 16)
        || BIT_EXTRACT_LE(stash->val_1_edx, 15, 16));
    intbool  prefetch = (BIT_EXTRACT_LE(stash->val_80000001_ecx, 8, 9)
...
// print some features, for AMD and HYGON processors
// stash = pointer to structure for accumulate processor information
...
// print multiprocessor information and processor model information, 
// as summary after CPUID functions results print
// raw   = flag for raw mode, if TRUE, summary information not printed
// debug = flag for print detail information include transit internal variables
// stash = pointer to structure for accumulate processor information
static void do_final(intbool raw, intbool debug, code_stash_t* stash)
{
    if (!raw) {
        ...
        void* p1 = &stash->mp;
        const struct mp* p2 = (const struct mp*)p1;
        print_mp_synth(p2);
        //
...
// print results of CPUID function 00000001h, register EAX = Processor signature: Type, Family, Model, Stepping
// value  = CPUID function 00000001h, register EAX
// vendor = processor vendor numeric id by enumeration
static void
print_1_eax(unsigned int value, vendor_t vendor)
{
...
// macro for extract CPU TFMS (Type, Family, Model, Stepping) signature field
#define B(b,str)                                   \
...
// print decoded CPU brand string, for INTEL processors
// val_eax = CPUID function 00000001h, register EAX
// val_ebx = CPUID function 00000001h, register EBX
static void
print_brand(unsigned int val_eax, unsigned int val_ebx)
{
...
// print results of CPUID function 00000001h, register EBX = SMP/CLFLUSH/Brand Index
// value = CPUID function 00000001h, register EBX
static void
...
// print results of CPUID function 00000001h, register ECX = Standard features bitmap (1 of 2)
// value = CPUID function 00000001h, register ECX
static void
...
// print results of CPUID function 00000001h, register EDX = Standard features bitmap (2 of 2)
// value = CPUID function 00000001h, register EDX
static void
...
// print results of CPUID function 00000002h, sequence of Cache and TLB descriptors bytes
// value     = one byte from descriptors bytes sequence CPUID function 00000002h
// vendor    = processor vendor numeric id by enumeration
// val_1_eax = CPUID function 00000001h, register EAX, as additional information
static void print_2_byte(unsigned char value, vendor_t vendor, unsigned int val_1_eax)
{
...
// print results of CPUID function 00000004h, deterministic cache information, register EAX
// value = CPUID function 00000004h, one of subfunctions, register EAX
// note for CPUID function 4: one subfunction used per each level of cache
static void
...
// print results of CPUID function 00000004h, deterministic cache information, register EBX
// value = CPUID function 00000004h, one of subfunctions, register EBX
// note for CPUID function 4: one subfunction used per each level of cache
static void
...
// print results of CPUID function 00000004h, deterministic cache information, register ECX
// value = CPUID function 00000004h, one of subfunctions, register ECX
// note for CPUID function 4: one subfunction used per each level of cache
static void
...
// print results of CPUID function 00000004h, deterministic cache information, register EDX
// value = CPUID function 00000004h, one of subfunctions, register EDX
// note for CPUID function 4: one subfunction used per each level of cache
static void
...
// print results of CPUID function 00000004h, deterministic cache information, calculated size of cache
// words = pointer to array of registers EAX, EBX, ECX, EDX after execution of one subfunction CPUID function 4
// note for CPUID function 4: one subfunction used per each level of cache
static void
...
// print results of CPUID function 00000005h, register EAX
// value = CPUID function 00000005h, register EAX
static void
...
// print results of CPUID function 00000005h, register EBX
// value = CPUID function 00000005h, register EBX
static void
...
// print results of CPUID function 00000005h, register ECX
// value = CPUID function 00000005h, register ECX
static void
...
// print results of CPUID function 00000005h, register EDX
// value = CPUID function 00000005h, register EDX
static void
...
// print results of CPUID function 00000006h, register EAX
// value = CPUID function 00000006h, register EAX
static void
...
// print results of CPUID function 00000006h, register EBX
// value = CPUID function 00000006h, register EBX
static void
...
// print results of CPUID function 00000006h, register ECX
// value = CPUID function 00000006h, register ECX
static void
...
// print results of CPUID function 00000006h, register EDX
// value = CPUID function 00000006h, register EDX
static void
...
// print results of CPUID function 00000007h, subfunction 0, register EBX
// value = CPUID function 00000007h, subfunction 0, register EBX
static void
...
// print results of CPUID function 00000007h, subfunction 0, register ECX
// value = CPUID function 00000007h, subfunction 0, register ECX
static void
...
// print results of CPUID function 00000007h, subfunction 0, register EDX
// value = CPUID function 00000007h, subfunction 0, register EDX
static void
...
// print results of CPUID function 00000007h, subfunction 1, register EAX
// value = CPUID function 00000007h, subfunction 1, register EAX
static void
...
// print results of CPUID function 0000000Ah, register EAX
// value = CPUID function 0000000Ah, register EAX
static void
...
// print results of CPUID function 0000000Ah, register EBX
// value = CPUID function 0000000Ah, register EBX
static void
...
// print results of CPUID function 0000000Ah, register ECX
// value = CPUID function 0000000Ah, register ECX
static void
...
// print results of CPUID function 0000000Ah, register EDX
// value = CPUID function 0000000Ah, register EDX
static void
...
// print results of CPUID functions 0000000Bh, 0000001Fh, register EAX
// value = CPUID function 0000000Bh or 0000001Fh, register EAX
// note this two functions has same data format
static void
...
// print results of CPUID functions 0000000Bh, 0000001Fh, register EBX
// value = CPUID function 0000000Bh or 0000001Fh, register EBX
// note this two functions has same data format
static void
...
// print results of CPUID functions 0000000Bh, 0000001Fh, register ECX
// value = CPUID function 0000000Bh or 0000001Fh, register ECX
// note this two functions has same data format
static void
...
// print results of CPUID function 0000000Dh, subfunction 0, register EAX
// value = CPUID function 0000000Dh, subfunction 0, register EAX
static void
...
// print results of CPUID function 0000000Dh, subfunction 1, register EAX
// value = CPUID function 0000000Dh, subfunction 1, register EAX
static void
...
// print results of CPUID function 0000000Dh, subfunction N, register ECX
// value = CPUID function 0000000Dh, one of subfunctions, register ECX
static void
...
// print results of CPUID function 0000000Dh, subfunction N
// words = pointer to array of registers EAX, EBX, ECX, EDX after execution of one subfunction CPUID function 0Dh
// tryX  = selector value for select one of CPU context elements
static void
print_d_n(const unsigned int words[WORD_NUM], unsigned int tryX)
{
    /*
    ...
    */
    ...
    ccstring  feature = features[tryX];
    int       feature_pad = 17 - (int)strlen(feature);
...
// print results of CPUID function 0000000Fh, subfunction 0, register EDX
// value = CPUID function 0000000Fh, subfunction 0, register EDX
static void
...
// print results of CPUID function 0000000Fh, subfunction 1, register EAX
// value = CPUID function 0000000Fh, subfunction 1, register EAX
static void
...
// print results of CPUID function 0000000Fh, subfunction 1, register EDX
// value = CPUID function 0000000Fh, subfunction 1, register EDX
static void
...
// print results of CPUID function 00000010h, subfunction 0, register EBX
// value = CPUID function 00000010h, subfunction 0, register EBX
static void
...
// print results of CPUID function 00000010h, subfunction N, register EAX
// value = CPUID function 00000010h, one of subfunctions, register EAX
static void
...
// print results of CPUID function 00000010h, subfunction N, register ECX
// value = CPUID function 00000010h, one of subfunctions, register ECX
static void
...
// print results of CPUID function 00000010h, subfunction N, register EDX
// value = CPUID function 00000010h, one of subfunctions, register EDX
static void
...
// print results of CPUID function 00000010h, subfunction 3, register EAX
// value = CPUID function 00000010h, subfunction 3, register EAX
static void
...
// print results of CPUID function 00000010h, subfunction 3, register ECX
// value = CPUID function 00000010h, subfunction 3, register ECX
static void
...
// print results of CPUID function 00000012h, subfunction 0, register EAX
// value = CPUID function 00000012h, subfunction 0, register EAX
static void
...
// print results of CPUID function 00000012h, subfunction 0, register EBX
// value = CPUID function 00000012h, subfunction 0, register EBX
static void
...
// print results of CPUID function 00000012h, subfunction 0, register EDX
// value = CPUID function 00000012h, subfunction 0, register EDX
static void
...
// print results of CPUID function 00000012h, subfunction N, register ECX
// value = CPUID function 00000012h, one of subfunctions, register ECX
static void
...
// print results of CPUID function 00000014h, subfunction 0, register EBX
// value = CPUID function 00000014h, subfunction 0, register EBX
static void
...
// print results of CPUID function 00000014h, subfunction 0, register ECX
// value = CPUID function 00000014h, subfunction 0, register ECX
static void
...
// print results of CPUID function 00000014h, subfunction 1, register EAX
// value = CPUID function 00000014h, subfunction 1, register EAX
static void
...
// print results of CPUID function 00000014h, subfunction 1, register EBX
// value = CPUID function 00000014h, subfunction 1, register EBX
static void
...
// print results of CPUID function 00000016h, register EAX
// value = CPUID function 00000016h, register EAX
static void
...
// print results of CPUID function 00000016h, register EBX
// value = CPUID function 00000016h, register EBX
static void
...
// print results of CPUID function 00000016h, register ECX
// value = CPUID function 00000016h, register ECX
static void
...
// print results of CPUID function 00000017h, subfunction 0, register EBX
// value = CPUID function 00000017h, subfunction 0, register EBX
static void
...
// print results of CPUID function 00000018h, subfunction N, register EBX
// value = CPUID function 00000018h, one of subfunctions, register EBX
static void
...
// print results of CPUID function 00000018h, subfunction N, register EDX
// value = CPUID function 00000018h, one of subfunctions, register EDX
static void
...
// print results of CPUID function 00000019h, register EAX
// value = CPUID function 00000019h, register EAX
static void
...
// print results of CPUID function 00000019h, register EBX
// value = CPUID function 00000019h, register EBX
static void
...
// print results of CPUID function 00000019h, register ECX
// value = CPUID function 00000019h, register ECX
static void
...
// print results of CPUID function 0000001Ah, subfunction 0, register EAX
// value = CPUID function 0000001Ah, subfunction 0, register EAX
static void
...
// print results of CPUID function 0000001Bh, subfunction N, register EAX
// value = CPUID function 0000001Bh, one of subfunctions, register EAX
static void
...
// print results of CPUID function 0000001Ch, register EAX
// value = CPUID function 0000001Ch, register EAX
static void
...
// print results of CPUID function 0000001Ch, register EBX
// value = CPUID function 0000001Ch, register EBX
static void
...
// print results of CPUID function 0000001Ch, register ECX
// value = CPUID function 0000001Ch, register ECX
static void
...
// print results of CPUID function 0000001Dh, subfunction N, register EAX
// value = CPUID function 0000001Dh, one of subfunctions, register EAX
static void
...
// print results of CPUID function 0000001Dh, subfunction N, register EBX
// value = CPUID function 0000001Dh, one of subfunctions, register EBX
static void
...
// print results of CPUID function 0000001Dh, subfunction N, register ECX
// value = CPUID function 0000001Dh, one of subfunctions, register ECX
static void
...
// print results of CPUID function 0000001Eh, register EBX
// value = CPUID function 0000001Eh, register EBX
static void
...
// print results of CPUID function 00000020h, register EBX
// value = CPUID function 00000020h, register EBX
static void
...
// print results of CPUID function 20000001h, register EDX
// value = CPUID function 20000001h, register EDX
static void
...
// print results of CPUID function 40000001h, register EAX
// value = CPUID function 40000001h, register EAX
static void
...
// print results of CPUID function 40000001h, register EDX
// value = CPUID function 40000001h, register EDX
static void
...
// print results of CPUID function 40000002h, register ECX
// value = CPUID function 40000002h, register ECX
static void
...
// print results of CPUID function 40000003h, register EAX
// value = CPUID function 40000003h, register EAX, for XEN
static void
...
// print results of CPUID function 40000003h, register EAX
// value = CPUID function 40000003h, register EAX, for Microsoft
static void
...
// print results of CPUID function 40000003h, register EBX
// value = CPUID function 40000003h, register EBX, for Microsoft
static void
...
// print results of CPUID function 40000003h, register ECX
// value = CPUID function 40000003h, register ECX, for Microsoft
static void
...
// print results of CPUID function 40000003h, register EDX
// value = CPUID function 40000003h, register EDX, for Microsoft
static void
...
// print results of CPUID function 40000004h, register EAX
// value = CPUID function 40000004h, register EAX, for XEN
static void
...
// print results of CPUID function 40000004h, register EAX
// value = CPUID function 40000004h, register EAX, for Microsoft
static void
...
// print results of CPUID function 40000005h, subfunction 0, register EBX
// value = CPUID function 40000005h, subfunction 0, register EBX, for XEN
static void
...
// print results of CPUID function 40000006h, register EAX
// value = CPUID function 40000006h, register EAX, for Microsoft
static void
...
// print results of CPUID function 40000007h, register EAX
// value = CPUID function 40000007h, register EAX, for Microsoft
static void
...
// print results of CPUID function 40000006h, register EBX
// value = CPUID function 40000007h, register EBX, for Microsoft
static void
...
// print results of CPUID function 40000008h, register EAX
// value = CPUID function 40000008h, register EAX, for Microsoft
static void
...
// print results of CPUID function 40000009h, register EAX
// value = CPUID function 40000009h, register EAX, for Microsoft
static void
...
// print results of CPUID function 40000009h, register EBX
// value = CPUID function 40000009h, register EBX, for Microsoft
static void
...
// print results of CPUID function 4000000Ah, register EAX
// value = CPUID function 4000000Ah, register EAX, for Microsoft
static void
...
// print results of CPUID function 80000001h, register EAX
// value = CPUID function 80000001h, register EAX, for AMD
static void
...
// print results of CPUID function 80000001h, register EAX
// value = CPUID function 80000001h, register EAX, for Hygon
static void
...
// print results of CPUID function 80000001h, register EAX
// value = CPUID function 80000001h, register EAX, for VIA
static void
...
// print results of CPUID function 80000001h, register EAX
// value = CPUID function 80000001h, register EAX, for Transmeta
static void
...
// print results of CPUID function 80000001h, register EAX
// value = CPUID function 80000001h, register EAX, for selected vendor
// vendor = processor vendor numeric id by enumeration
static void
print_80000001_eax(unsigned int value, vendor_t vendor)
{
    switch (vendor) {
    case VENDOR_AMD:
...
// print results of CPUID function 80000001h, register EDX
// value = CPUID function 80000001h, register EDX, for Intel
static void
...
// print results of CPUID function 80000001h, register EDX
// value = CPUID function 80000001h, register EDX, for AMD
static void
...
// print results of CPUID function 80000001h, register EDX
// value = CPUID function 80000001h, register EDX, for Cyrix or VIA
static void
...
// print results of CPUID function 80000001h, register EDX
// value = CPUID function 80000001h, register EDX, for Transmeta
static void
...
// print results of CPUID function 80000001h, register EDX
// value = CPUID function 80000001h, register EDX, for NSC
static void
...
// print results of CPUID function 80000001h, register EDX
// value = CPUID function 80000001h, register EDX, for selected vendor
// vendor = processor vendor numeric id by enumeration
static void
print_80000001_edx(unsigned int value, vendor_t vendor)
{
    switch (vendor) {
    case VENDOR_INTEL:
...
// print results of CPUID function 80000001h, register ECX
// value = CPUID function 80000001h, register ECX, for AMD
static void
...
// print results of CPUID function 80000001h, register ECX
// value = CPUID function 80000001h, register ECX, for Intel
static void
...
// print results of CPUID function 80000001h, register ECX
// value = CPUID function 80000001h, register ECX, for selected vendor
// vendor = processor vendor numeric id by enumeration
static void
print_80000001_ecx(unsigned int value, vendor_t vendor)
{
    switch (vendor) {
    case VENDOR_AMD:
...
// print results of CPUID function 80000001h, register EBX
// value = CPUID function 80000001h, register EBX, for AMD
static void
print_80000001_ebx_amd(unsigned int value, unsigned int val_1_eax)
{
...
// print results of CPUID function 80000001h, register EBX
// value     = CPUID function 80000001h, register EBX, for selected vendor
// vendor    = processor vendor numeric id by enumeration
// val_1_eax = CPUID function 00000001h, register EAX
static void
print_80000001_ebx(unsigned int value, vendor_t vendor, unsigned int val_1_eax)
{
    switch (vendor) {
    case VENDOR_AMD:
    case VENDOR_HYGON:
...
// print results of CPUID function 80000005h, register EAX
// value = CPUID function 80000005h, register EAX
static void
...
// print results of CPUID function 80000005h, register EBX
// value = CPUID function 80000005h, register EBX
static void
...
// print results of CPUID function 80000005h, register ECX
// value = CPUID function 80000005h, register ECX
static void
...
// print results of CPUID function 80000005h, register EDX
// value = CPUID function 80000005h, register EDX
static void
...
// strings array for decode cache associativity
...
// print results of CPUID function 80000006h, register EAX
// value = CPUID function 80000006h, register EAX
static void
...
// print results of CPUID function 80000006h, register EBX
// value = CPUID function 80000006h, register EBX
static void
...
// print results of CPUID function 80000006h, register ECX
// value = CPUID function 80000006h, register ECX
static void
print_80000006_ecx(unsigned int value, code_stash_t* stash)
{
    static named_item  names[]
        = { { "line size (bytes)"                       ,  0,  7, NIL_IMAGES },
...
// print results of CPUID function 80000006h, register EDX
// value = CPUID function 80000006h, register EDX
static void
...
// print results of CPUID function 80000007h, register EBX
// value = CPUID function 80000007h, register EBX
static void
...
// print results of CPUID function 80000007h, register ECX
// value = CPUID function 80000007h, register ECX
static void
...
// print results of CPUID function 80000007h, register EDX
// value = CPUID function 80000007h, register EDX
static void
...
// print results of CPUID function 80000008h, register EAX
// value = CPUID function 80000008h, register EAX
static void
...
// print results of CPUID function 80000008h, register EBX
// value = CPUID function 80000008h, register EBX
static void
...
// print results of CPUID function 80000008h, register ECX
// value = CPUID function 80000008h, register ECX
static void
...
// print results of CPUID function 80000008h, register EDX
// value = CPUID function 80000008h, register EDX
static void
...
// print results of CPUID function 8000000Ah, register EAX
// value = CPUID function 8000000Ah, register EAX
static void
...
// print results of CPUID function 8000000Ah, register EDX
// value = CPUID function 8000000Ah, register EDX
static void
...
// print results of CPUID function 8000000Ah, register EBX
// value = CPUID function 8000000Ah, register EBX
static void
...
// print results of CPUID function 80000019h, register EAX
// value = CPUID function 80000019h, register EAX
static void
...
// print results of CPUID function 80000019h, register EBX
// value = CPUID function 80000019h, register EBX
static void
...
// print results of CPUID function 8000001Ah, register EAX
// value = CPUID function 8000001Ah, register EAX
static void
...
// print results of CPUID function 8000001Bh, register EAX
// value = CPUID function 8000001Bh, register EAX
static void
...
// print results of CPUID function 8000001Ch, register EAX
// value = CPUID function 8000001Ch, register EAX
static void
...
// print results of CPUID function 8000001Ch, register EBX
// value = CPUID function 8000001Ch, register EBX
static void
...
// print results of CPUID function 8000001Ch, register ECX
// value = CPUID function 8000001Ch, register ECX
static void
...
// print results of CPUID function 8000001Ch, register EDX
// value = CPUID function 8000001Ch, register EDX
static void
...
// print results of CPUID function 8000001Dh, register EAX
// value = CPUID function 8000001Dh, register EAX
static void
...
// print results of CPUID function 8000001Dh, register EBX
// value = CPUID function 8000001Dh, register EBX
static void
...
// print results of CPUID function 8000001Dh, register EDX
// value = CPUID function 8000001Dh, register EDX
static void
...
// print results of CPUID function 8000001Dh, calculated parameters, cache size as summary
// words = pointer to array of registers EAX, EBX, ECX, EDX after 
// execution of one subfunction CPUID function 8000001Dh
// note for CPUID function 8000001Dh: one subfunction used per each level of cache
// note CPUID function 8000001Dh is AMD-variant of Intel function 00000004h, deterministic cache detection
static void
...
// print results of CPUID function 8000001Eh, register EBX
// value = CPUID function 8000001Eh, register EBX
static void
...
// print results of CPUID function 8000001Eh, register ECX
// value = CPUID function 8000001Eh, register ECX
static void
...
// print results of CPUID function 8000001Fh, register EAX
// value = CPUID function 8000001Fh, register EAX
static void
...
// print results of CPUID function 8000001Fh, register EBX
// value = CPUID function 8000001Fh, register EBX
static void
...
// print results of CPUID function 80000020h, subfunction 0, register EBX
// value = CPUID function 80000020h, subfunction 0, register EBX
static void
...
// print results of CPUID function 80860001h, register EAX
// value = CPUID function 80860001h, register EAX
static void
...
// print results of CPUID function 80860001h, register EDX
// value = CPUID function 80860001h, register EDX
static void
...
// print TRANSMETA processor revision
// proc_rev = revision code
static void
...
// print results of CPUID function 80860001h, registers EBX, ECX
// val_ebx = CPUID function 80860001h, register EBX
// val_ecx = CPUID function 80860001h, register ECX
static void
print_80860001_ebx_ecx(unsigned int val_ebx, unsigned int  val_ecx)
{
...
// print results of CPUID function 80860002h, register EAX
// value = CPUID function 80860002h, register EAX
// stash = collection of vendor-specific and device-specific information after CPUID functions execution
static void
print_80860002_eax(unsigned int value, code_stash_t* stash)
{
...
// print results of CPUID function C0000001h, register EDX
// value = CPUID function C0000001h, register EDX
static void
...
// print results of CPUID function C0000002h, register EBX
// value = CPUID function C0000002h, register EBX
static void
...
// print results of CPUID function C0000002h, register ECX
// value = CPUID function C0000002h, register ECX
static void
...
// print results of CPUID function C0000002h, register EDX
// value = CPUID function C0000002h, register EDX
static void
...
// print results of CPUID function C0000004h, register EAX
// value = CPUID function C0000004h, register EAX
static void
...
// print results of CPUID function C0000004h, register EBX
// value = CPUID function C0000004h, register EBX
static void
...
// print results of CPUID function C0000004h, register ECX
// value = CPUID function C0000004h, register ECX
static void
...
// macro for extract data from text string
...
// print CPU registers as raw dump
// reg   = CPUID function number
// tryX  = CPUID sub-function number
// words = array of EAX, EBX, ECX, EDX values after CPUID function:subfunction
static void
print_reg_raw(unsigned int reg, unsigned int tryX, const unsigned int words[WORD_NUM])
{
...
// print CPU registers as decoded CPUID information
// reg   = CPUID function number, select information interpreter
// words = pointer to array of registers EAX, EBX, ECX, EDX after execution of one function:subfunction
// raw   = flag for print raw data, without decoding
// tryX  = CPUID subfunction number
// stash = collection of vendor-specific and device-specific information after CPUID functions execution
static void
print_reg(unsigned int reg, const unsigned int words[WORD_NUM], intbool raw, unsigned int tryX, code_stash_t* stash)
{
    if (reg == 0) {
        ...
        stash->saw_b = TRUE;
        if (tryX < LENGTH(stash->val_b_eax)) {
            stash->val_b_eax[tryX] = words[WORD_EAX];
        }
        if (tryX < LENGTH(stash->val_b_ebx)) {
            stash->val_b_ebx[tryX] = words[WORD_EBX];
        }
    }
    else if (reg == 0x1f) {
        stash->saw_1f = TRUE;
        if (tryX < LENGTH(stash->val_1f_eax)) {
            stash->val_1f_eax[tryX] = words[WORD_EAX];
        }
        if (tryX < LENGTH(stash->val_1f_ebx)) {
            stash->val_1f_ebx[tryX] = words[WORD_EBX];
        }
        if (tryX < LENGTH(stash->val_1f_ecx)) {
            stash->val_1f_ecx[tryX] = words[WORD_ECX];
        }
    }
    ...
    if (raw) {
        ...
        for (; word < 4; word++) {
            if ((words[word] & 0x80000000) == 0) {
                ...
                unsigned int          byte = (tryX == 0 && word == WORD_EAX ? 1
                    : 0);
                for (; byte < 4; byte++) {
        ...
        else if (tryX >= 2 && tryX < 63) {
            print_d_n(words, tryX);
        }
        else {
            print_reg_raw(reg, tryX, words);
        }
        ...
        printf("      identifier of target %d = 0x%08x (%u)\n",
            3 * tryX + 1, words[WORD_EBX], words[WORD_EBX]);
        printf("      identifier of target %d = 0x%08x (%u)\n",
            3 * tryX + 2, words[WORD_ECX], words[WORD_ECX]);
        printf("      identifier of target %d = 0x%08x (%u)\n",
            3 * tryX + 3, words[WORD_EDX], words[WORD_EDX]);
    }
    else if (reg == 0x1c) {
        ...
        if (stash->vendor == VENDOR_VIA) {
            ...
            printf("   0x%08x 0x%02x: eax=0x%08x\n",
                (unsigned int)reg, tryX, words[WORD_EAX]);
...
// this helper added for WinAPI support when porting from Linux to Windows
// get number of processors by OS API
static int
processors_count_helper()
{
    SYSTEM_INFO sysinfo;
...
// prepare OS for execute CPUID: set affinity mask for select CPU
// cpu     = CPU selection number
// one_cpu = flag for single CPU mode selected by user
// inst    = flag for instruction mode selected by user
// return mode selection code, yet used -1 = Error, USE_INSTRUCTION = use CPUID indtruction
static int
real_setup(unsigned int cpu, intbool one_cpu, intbool inst)
{
    if (inst) {
        if (!one_cpu) {
            /*
            ...
            */
            ...
            if (num_cpus == 0) {
                ...
                num_cpus = processors_count_helper();          // porting changes: replace Linux API to Windows API

            }
            if (cpu >= num_cpus) return -1;

#ifdef USE_KERNEL_SCHED_SETAFFINITY
            // start of fragment, yet not ported from Linux to Windows
                        /*
            ...
            //
            // end of fragment, yet not ported from Linux to Windows
#endif
//
...
// start of porting fragment, yet for single processor group only (yet max. 64 logical processors)

            // affinization variant 1, not effective
            //HANDLE hThread = GetCurrentThread();
            //int status = -1;
            //if (hThread != NULL)
            //    status = SetThreadIdealProcessor(hThread, cpu);

            // affinization variant 2, effective
            HANDLE hThread = GetCurrentThread();
            DWORD_PTR status = -1;
            DWORD_PTR cpumask = 1;
            cpumask <<= cpu;
            if (hThread != NULL)
                status = SetThreadAffinityMask(hThread, cpumask);

            // end of porting fragment, yet for single processor group only (yet max. 64 logical processors)
            // #endif
            ...
            Sleep(0);    // porting from Linux to Windows
...
// Get CPUID data, yet ported one method only: direct execute CPUID instruction
// file method (get data from text file) YET NOT SUPPORTED.
// reg = function code for CPUID instruction, input EAX for CPUID instruction
// ecx = sub-function code for CPUID instruction, input ECX for CPUID instruction
// words[] = array of 4 32-bit double words, return EAX, EBX, ECX, EDX registers after CPUID execution
static int real_get(int cpuid_fd, unsigned int reg, unsigned int words[], unsigned int ecx, intbool quiet)
{
    if (cpuid_fd == USE_INSTRUCTION)
    {
        
#if defined(__GNUC__)
        __cpuid_count(reg, ecx, words[0], words[1], words[2], words[3]);      // use GNU intrinsic
#else
        __cpuidex((int*)words, reg, ecx);                                     // use Microsoft intrinsic
#endif
    ...
    /*
    // yet ported one method only: direct execute CPUID instruction
    // file method (get data from text file) YET NOT SUPPORTED.
...
// print CPUID function name (information header)
// reg  = CPUID function number
// tryX = CPUID sub-function number
// raw  = flag for raw dump without decoding data, no prints if raw mode selected
static void
print_header(unsigned int reg, unsigned int tryX, intbool raw)
{
    if (!raw) {
        if (reg == 2 && tryX == 0) {
            printf("   cache and TLB information (2):\n");
        }
        else if (reg == 4 && tryX == 0) {
            printf("   deterministic cache parameters (4):\n");
        }
        else if (reg == 7 && tryX == 0) {
            printf("   extended feature flags (7):\n");
        }
        else if (reg == 0xb && tryX == 0) {
            printf("   x2APIC features / processor topology (0xb):\n");
        }
        else if (reg == 0x1d && tryX == 0) {
            printf("   Tile Information (0x1d/0):\n");
        }
        else if (reg == 0x1f && tryX == 0) {
            printf("   V2 extended topology (0x1f):\n");
        }
        else if (reg == 0x20 && tryX == 0) {
            printf("   Processor History Reset information (0x20):\n");
        }
        else if (reg == 0x40000003 && tryX == 0) {
            printf("   hypervisor time features (0x40000003/00):\n");
        }
        else if (reg == 0x40000003 && tryX == 1) {
            printf("   hypervisor time scale & offset (0x40000003/01):\n");
        }
        else if (reg == 0x40000003 && tryX == 2) {
            printf("   hypervisor time physical cpu frequency (0x40000003/02):\n");
        }
        else if (reg == 0x8000001d && tryX == 0) {
            printf("   Cache Properties (0x8000001d):\n");
        }
    }
}

// Print CPUID data for one leaf (means one function)
// reg     = CPUID function number
// tryX    = CPUID sub-function number
// one_cpu = flag for single CPU mode, not duplicate CPUID execution by logical processors
// inst    = flag for instruction mode, use CPUID instruction on physical platform
//           INSTRUCTION MODE IS DEFAULT MODE, ONLY THIS MODE YET SUPPORTED
// raw     = flag for raw dump without decoding data, no prints if raw mode selected
// debug   = flag for debug mode, print detail transit info
static void
do_real_one(unsigned int reg, unsigned int tryX, intbool one_cpu, intbool inst, intbool raw, intbool debug UNUSED)
{
    unsigned int  cpu;
    ...
    for (cpu = 0;; cpu++) {
        ...
        real_get(cpuid_fd, reg, words, tryX, FALSE);
        print_reg(reg, words, raw, tryX, &stash);
    }
}

// Print CPUID data, yet ported one method only: direct execute CPUID instruction
// file method (get data from text file) YET NOT SUPPORTED.
// one_cpu = flag for single CPU mode, not duplicate CPUID execution by logical processors
// inst    = flag for instruction mode, use CPUID instruction on physical platform
//           INSTRUCTION MODE IS DEFAULT MODE, ONLY THIS MODE YET SUPPORTED
...
// debug   = flag for debug mode, print detail transit info
static void
do_real(intbool one_cpu, intbool inst, intbool raw, intbool debug)
{
    unsigned int  cpu;
    ...
    for (cpu = 0;; cpu++) {
        ...
        // enumerate standard CPUID functions at theoretical range 00000000h - 3FFFFFFFh
        max = 0;
        for (reg = 0; reg <= max; reg++) {
            ...
            if (reg == 0) {
                max = words[WORD_EAX];  // update maximum function number
            }
            ...
            if (reg == 2) {
                ...
                print_header(reg, tryX, raw);

                for (;;) {
                    print_reg(reg, words, raw, tryX, &stash);

                    tryX++;
                    if (tryX >= max_tries) break;
                ...
                */
                ...
                unsigned int  tryX;
                for (tryX = 2; tryX < 63; tryX++) {
                    if (valid_tries & (1ull << tryX)) {
                        real_get(cpuid_fd, reg, words, tryX, FALSE);
                        print_reg(reg, words, raw, tryX, &stash);
                    }
                }
        ...
        // enumerate virtual CPUID functions at theoretical range 40000000h - 7FFFFFFFh
        ...
        // enumerate Intel Xeon Phi specific CPUID functions at theoretical range 20000000h - 200000FFh
        max = 0x20000000;
        ...
        // enumerate extended CPUID functions at theoretical range 80000000h - FFFFFFFFh
        max = 0x80000000;
        ...
        // enumerate Transmeta specific CPUID functions from 80860000h
        max = 0x80860000;
        ...
        // enumerate VIA specific CPUID functions from C0000000h
        max = 0xc0000000;
...
// Print CPUID data, yet ported one method only: direct execute CPUID instruction
// THIS file method (get data from text file) YET NOT SUPPORTED.
// filename = file name string
// raw      = flag for raw dump without decoding data, no prints if raw mode selected
// debug    = flag for debug mode, print detail transit info
static void
do_file(ccstring filename, intbool raw, intbool debug)
{
    intbool       seen_cpu = FALSE;
    unsigned int  cpu = -1;
    /*
...
// command line parameters interpreter,
// count = same as main input argc = number of command line parameters, include parameters[0] = application exe file name
// options = same as main input argv = array of strings, command line parameters
// shortcontrol = pointer to chars array for short (single char) options detection, for example "-h"
// longcontrol = pointer to structures array for long (full name) options detection, for example "--help"
// foundindex = pointer to returned index of detected option
// return detected option key char, or '?' if not detected
int
emulate_getopt_long(int count, string* options, ccstring shortcontrol, const option* longcontrol, int* foundindex)
{

    // detect options count done or continue parse
    if (emulate_optind < count)
    {
        string s1 = options[emulate_optind];  // pointer to command line option string
        string s0 = s1;
        cstring s2 = (cstring)shortcontrol;   // pointer to short options chars list control string
        option* op = (option*)longcontrol;    // pointer to array of long options control structures
        int c1 = *s1++;   // analysed char from command line
        int c2 = 0;       // next analysed char from command line
        int c3 = 0;       // control char from short options chars list control string
        int c4 = 0;       // next control char from short options chars list control string
        bool b = false;   // set to TRUE if option handled as short (single char)

        emulate_optarg = NULL;

        // detect wrong options, not start with "="
        if ((c1 != '-') || (c1 == 0))
        {
            c1 = '?';
            emulate_optopt = 0;
        ...
        // detect short options, start with "-" or long options, start with "--"
        else
        {
            c1 = *s1++;

            // detect short options, start with "-" but not "--"
            if ((c1 != 0) && (c1 != '-'))
            {
                c2 = *s1++;
                if (c2 == 0)
                {
                    ...
                    while (c3 != 0)
                    {
                        if (c3 == c1)
                        {
                            ...
                            if (c4 == ':')
                            {
                                emulate_optind++;
                                if (emulate_optind < count)
                                    emulate_optarg = options[emulate_optind];
                            }
                            break;
                        }
                        c3 = *s2++;
                    ...
                    if (c3 != c1)
                        c1 = '?';
                    b = true;  // mark handled as short
                }
            }

            // detect long options, start with "--"
            if (!b)
            {
                s0 = s0 + 2;   // +2 for skip "--"
                cstring ls = op->name;
                ...
                // check for termination by "=" as parameter separator or 0 as string terminator
                size_t n = strlen(s0);
                s1 = strchr(s0, '=');
                if (s1 != NULL)
                    n = s1 - s0;

                // compare ls=template and s1=option
                while (ls != NULL)
                {
                    cmp = strncmp(ls, s0, n);
                    if (cmp == 0)
                ...
                if (cmp == 0)
                {
                    emulate_optopt = op->val;  // handling strings match
                    c1 = op->val;
                    if ((op->has_arg == TRUE) && (s1 != NULL))
                    {
                        emulate_optarg = s1 + 1;  // +1 for skip "="
                    }
                }
                else
                {
                    emulate_optopt = 0;  // handling strings mismatch
                    c1 = '?';
                }
            }
        }

        // return when current option parsed
        emulate_optind++;
        return c1;
    }

    // return if options count done
    else
    {
        return EOF;
    }
...
// argc = number of command line parameters, include parameters[0] = application exe file name
...
main(int argc, string  argv[])
{
    ...
    // options flags, set to TRUE when option usage detected by command line parse
    intbool  opt_one_cpu = FALSE;  // execute CPUID for current thread only, default is execute CPUID for all detected logical processors, "-1" or "--one-cpu"
    intbool  opt_inst = FALSE;     // execute CPUID instruction direct, alternative of kernel module or file mode, this is default, "-i" or "--inst"
    intbool  opt_kernel = FALSE;   // use kernel module, yet not supported by windows port, "-k" or "--kernel"
    intbool  opt_raw = FALSE;      // raw output hex dump data, CPUID instruction, registers EAX, EBX, ECX, EDX, "-r" or "--raw"
    intbool  opt_debug = FALSE;    // debug mode, show detail internal info, this mode keys yet not listed at help, "-d" or "--debug"
    intbool  opt_file = FALSE;     // interpreting CPUID information from text file, default is execute CPUID on platform, "f FILENAME" or "--file=FILENAME"
    intbool  opt_version = FALSE;  // output version of original linux application and this windows port, "-v" or "--version"
    intbool  opt_leaf = FALSE;     // execute CPUID instruction only for specified leaf (CPUID function, input EAX), "-l NUMBER" or "leaf=NUMBER"
    intbool  opt_subleaf = FALSE;  // execute CPUID instruction only for specified subleaf (CPUID sub-function, input ECX), "-s NUMBER" or "--subleaf=NUMBER"

    cstring        opt_filename = NULL;    // pointer to file name, used for file mode
    unsigned long  opt_leaf_val = 0;       // CPUID instruction function number (same as input EAX), for single leaf mode
    unsigned long  opt_subleaf_val = 0;    // CPUID instruction sub-function number (same as input ECX), for single sub-leaf mode

    program = strrchr(argv[0], '\\');      // extract application exe file name (skip path) for text messages
    if (program == NULL) {
        program = argv[0];
    }
    else {
    ...
    // start cycle for parse command line arguments
    for (;;) {
        int  longindex = 0;
        int  opt = emulate_getopt_long(argc, argv, shortopts, longopts, &longindex);
        ...
        case 'l':
            opt_leaf = TRUE;
            {
                emulate_errno = 0;
                char* endptr = NULL;
                opt_leaf_val = strtoul(emulate_optarg, &endptr, 0);
                if (emulate_errno != 0) {
                    fprintf(stderr,
                        "%s: argument to -l/--leaf not understood: %s\n",
                        program, argv[emulate_optind - 1]);
                    exit(1);
                }
            }
            break;
        case 's':
            opt_subleaf = TRUE;
            {
                errno = 0;
                char* endptr = NULL;
                opt_subleaf_val = strtoul(emulate_optarg, &endptr, 0);
                if (errno != 0) {
                    fprintf(stderr,
        ...
        default:
            if (emulate_optopt == '\0') {
                ...
                fprintf(stderr,
                    "%s: unrecognized option letter: %c\n", program, emulate_optopt);
            }
            usage();
            // NOTREACHED
        }
    }
    // end cycle for  parse command line arguments, next, start interpreting options

    // check for condition: all arguments parsed
    if (emulate_optind < argc) {
        fprintf(stderr, "%s: unrecognized argument: %s\n", program, argv[emulate_optind]);
        usage();
        // NOTREACHED
    }

    // detect error: use kernel option when not available
    ...
    // detect error: use instruction and kernel options simultaneously
    ...
    // detect error: use file and leaf options simultaneously
    ...
    // detect error: subleaf specified without leaf
    ...
    // detect error: file mode yet not supported, this temporary added for Windows Port version
    if (opt_file) {
        fprintf(stderr,
            "%s: -f/--file option yet not supported by windows port version\n",
            program);
        exit(1);
    }

    // detect error: instruction mode option yet not supported, 
    // because for current version instruction mode is only one supported mode
    // this temporary added for Windows Port version
    if (opt_inst) {
        fprintf(stderr,
            "%s: -i/--inst option yet not supported by windows port version,\nnote only instruction mode yet supported, this is default\n",
            program);
        exit(1);
    }


    // errors and warnings detection done,
    // Default to -i.  So use inst unless -k is specified.
    intbool  inst = !opt_kernel;

    // if version option - show version
    if (opt_version) {
        printf(VERSION_TEXT);
    }
    ...
    // execute cpuid
        ...
        if (opt_filename != NULL) {
            do_file(opt_filename, opt_raw, opt_debug);         // file mode, YET NOT SUPPORTED
        }
        else if (opt_leaf) {
            do_real_one(opt_leaf_val, opt_subleaf_val,         // one selected function mode, from physical platform
                opt_one_cpu, inst, opt_raw, opt_debug);
        }
        else {
            do_real(opt_one_cpu, inst, opt_raw, opt_debug);    // execute all supported functions, from physical platform, this is DEFAULT mode
        }
    }
